...
if (!window.crypto && (window as any).msCrypto) {
  ...
  const legacyDigest = (window as any).msCrypto.subtle.digest;
  // Delete the property to prevent a circular reference
  delete (window as any).msCrypto.subtle.digest;

  // Wrap the cached digest method in a Promise
  const modernDigest = (type: any, array: any) => {
    return new Promise((resolve, reject) => {
      // Use `call` here to ensure the proper `this` binding
      const cryptoObj = legacyDigest.call((window as any).msCrypto.subtle, type, array);
      ...
      cryptoObj.oncomplete = (evt: { target: { result: any } }) => {
  ...
  (window.crypto as any).subtle.digest = modernDigest;
}
...
(async function () {
  ...
  const unCb = (step as forgerock.FRStep).getCallbackOfType(
    'ValidatedCreateUsernameCallback' as forgerock.CallbackType,
  );
